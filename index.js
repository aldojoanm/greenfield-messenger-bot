...
function markSource(s, src) {
  s.meta = s.meta || {};
  if (!s.meta.source) s.meta.source = src;
}
  ...
  const quick_replies = (options || []).slice(0, 11).map(o => {
    if (typeof o === 'string') {
      return {
        content_type: 'text',
        title: clamp(o),
        payload: `QR_${o.replace(/\s+/g, '_').toUpperCase()}`,
      };
    }
    return {
      content_type: 'text',
      title: clamp(o.title),
      payload: o.payload || `QR_${o.title.replace(/\s+/g, '_').toUpperCase()}`,
    };
  });
      ...
      attachment: {
        ...
        payload: {
          ...
          buttons: buttons
            ...
            .map(b => {
              if (b.type === 'web_url') {
                return { type: 'web_url', url: b.url, title: clamp(b.title) };
              }
              if (b.type === 'postback') {
                return {
                  type: 'postback',
...
async function ensureProfileName(psid) {
  ...
  if (s.meta?.profileFetchFailed) return null;

  const n = await fetchFBProfileName(psid);
  if (n) s.profileName = n;
  else {
    s.meta = s.meta || {};
    s.meta.profileFetchFailed = true;
  }
...
async function finishAndWhatsApp(psid) {
  ...
  s.flags.finalShown = true;
  s.flags.finalShownAt = Date.now();
  ...
  if (prod) {
    ...
    await sendText(
      ...
      `¡Excelente! Sobre *${prod.nombre}* puedo ayudarte con una cotización. ` +
        `Para enviarte una cotización sin compromiso, podrías ayudarme con unos datos rápidos.`
    );
    await nextStep(psid);
    return true;
  }
    ...
    for (const entry of req.body.entry || []) {
      for (const ev of entry.messaging || []) {
        ...
        if (qr) {
          ...
          if (qr === 'OPEN_CATALOG') {
            await sendButtons(psid, 'Abrir catálogo completo', [
              { type: 'web_url', url: CATALOG_URL, title: 'Ver catálogo' },
            ]);
          ...
          if (qr === 'OPEN_LOCATION') {
            await sendButtons(psid, 'Nuestra ubicación en Google Maps 👇', [
              { type: 'web_url', url: linkMaps(), title: 'Ver ubicación' },
            ]);
            await showHelp(psid);
            continue;
          }
          ...
          if (qr === 'OPEN_WHATSAPP') {
            const wa = whatsappLinkFromSession(s);
            if (wa) {
              await sendButtons(psid, 'Te atiende un asesor por WhatsApp 👇', [
          ...
          if (/^DPTO_/.test(qr)) {
            ...
            s.vars.departamento = dep;
            s.vars.subzona = null;
            s.pending = null;
            if (dep === 'Santa Cruz') await askSubzonaSCZ(psid);
        ...
        if (ha) s.vars.hectareas = ha;
        const phone = parsePhone(text);
        if (phone) s.vars.phone = phone;
        ...
        if (!s.vars.departamento || s.pending === 'departamento') {
          ...
          if (depTyped) {
            s.vars.departamento = depTyped;
            s.vars.subzona = null;
            s.pending = null;
            if (depTyped === 'Santa Cruz') await askSubzonaSCZ(psid);
            else await askSubzonaLibre(psid);
            continue;
        ...
        if (s.vars.departamento === 'Santa Cruz' && (!s.vars.subzona || s.pending === 'subzona')) {
          ...
          if (s.pending === 'subzona') {
            await askSubzonaSCZ(psid);
            continue;
          }
        }
        ...
        if (wantsCatalog(text)) {
          ...
          await sendText(psid, '¿Qué *producto* te interesó del catálogo?');
          s.pending = 'prod_from_catalog';
          await showHelp(psid);
          continue;
        }
        ...
        if (asksPrice(text)) {
          ...
          await sendText(
            psid,
            'Con gusto te preparamos una *cotización*. Primero confirmemos tu ubicación para asignarte el asesor correcto.'
          );
          await nextStep(psid);
          continue;
        }
